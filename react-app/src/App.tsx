...
import './App.css';
  ...
  // Row-level actions
  const onEdit = (name: string) => alert(`Edit user ${name}`);
  ...
  return (
    <div className="App">
      ...
      <p className="description">Sorting, filtering, pagination, resizing, actions, theming, and state persistence.</p>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
        ...
        />
        <label>
          <input type="checkbox" checked={wrapText} onChange={(e) => setWrapText(e.target.checked)} /> Wrap text
        </label>
        <label>
          Density:
        ...
        <div style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}>
          <span>Columns:</span>
          {(['name','email','role','status','createdAt'] as const).map((key) => (
            <label key={key}>
              <input
                type="checkbox"
        ...
        <button onClick={simulateLoading} disabled={loading}>Simulate loading</button>
      ...
      <div className="ag-theme-quartz" style={{ height: 540, width: '100%', margin: '0 auto' }}>
        <AgGridReact
          ref={gridRef}
          rowData={rowData}
          ...
          // Horizontal scroll appears when total col width exceeds container
        />
      </div>
    </div>
  );
